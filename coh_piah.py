...
def compara_assinatura(as_a, as_b):
    ...
    for i in range(0, 6):
        ...
        if subtração < 0:
            subtração = abs(subtração)
        somatoria = somatoria + subtração
    s_ab = somatoria / 6
    return s_ab
...
def calcula_assinatura(texto):
    ...
    n_palavras = len(lista_de_palavras)

    tamanho_palavras = 0
    for i in lista_de_palavras:
        tamanho_palavras = tamanho_palavras + len(i)

    tamanho_medio_palavras = tamanho_palavras / n_palavras

    type_token = n_palavras_diferentes(texto) / n_palavras

    hapax_legomana = n_palavras_unicas(texto) / n_palavras

    n_sentencas = len(separa_sentencas(texto))

    n_frases = len(separa_frases(texto))

    tamanho_medio_sentenca = tamanho_palavras / n_sentencas

    complexidade_sentenca = n_frases / n_sentencas

    tamanho_medio_frase = tamanho_palavras / n_frases

    assinatura = [tamanho_medio_palavras, type_token, hapax_legomana,
